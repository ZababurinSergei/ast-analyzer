...
// Версия: 5.2.0 - с поддержкой gr.e, gr.re, gr.ce, flFull
// ✅ ИСПРАВЛЕНО: конфликт FLAG_MAP (2048: 'a' → 'A')
// ✅ ИСПРАВЛЕНО: collectReExports принимает глобальные карты
// ✅ ИСПРАВЛЕНО: compressPaths сохраняет полные пути в flFull
// ✅ ДОБАВЛЕНО: decodeFlagString, decodeFlagObject, FLAG_CHAR_TO_BIT
...
/**
 * ✅ ИСПРАВЛЕНО: конфликт '2048': 'a' устранён
 * Бит 2048 (async chain) теперь даёт 'A' (заглавная)
 */
export const FLAG_MAP = {
    ...
    '512': 'x', // external
    '1024': 't', // vue template
    '2048': 'A', // async chain ← ИСПРАВЛЕНО: было 'a'
    '4096': 'l', // closure
    '8192': 'y', // type dependency
};

/**
 * ✅ НОВОЕ: обратная карта символ → бит
 * Используется для декодирования строк флагов
 */
...
/**
 * ✅ ИСПРАВЛЕНО: безопасное сжатие путей
 * Сохраняет полные пути в отдельное поле flFull
 */
function compressPaths(data: any): any {
    ...
    if (data.fl && typeof data.fl === 'object') {
        ...
        for (const [key, value] of Object.entries(data.fl)) {
            ...
            fullPaths[key] = pathStr; // ← сохраняем полный путь
            ...
            if (parts.length > 4) {
                ...
                compressed[key] = pathStr;
            }
        }

        data.fl = compressed;
        data.flFull = fullPaths; // ← добавляем полные пути
    }
    return data;
}
...
/**
 * ✅ НОВОЕ: декодирует строку флагов в массив имён
 * @param flagsStr - строка типа "es", "a", "0", "aA"
 * @returns массив имён флагов
 */
export function decodeFlagString(flagsStr: string): string[] {
...
/**
 * ✅ НОВОЕ: декодирует строку флагов в объект с boolean
 * @param flagsStr - строка типа "es", "a", "0"
 * @returns объект с булевыми полями
 */
export function decodeFlagObject(flagsStr: string): Record<string, boolean> {
...
// ============================================
// ✅ ИСПРАВЛЕНО: СБОР РЕЭКСПОРТОВ С ГЛОБАЛЬНЫМИ КАРТАМИ
// ============================================
 ...
 * @param moduleMap - глобальная карта модулей (из generateCompactReport)
 * @param globalFuncIndex - глобальный индекс функций по имени
 * @returns массив реэкспортов в формате [moduleIdx, funcIdx, source, name, line, type]
 */
function collectReExports(
    entitiesMap: Record<string, EntitiesResult>,
    moduleMap: Map<string, string>,
    globalFuncIndex: Map<string, { fileId: string; funcId: string; idx: number; isExported: boolean }>
): any[] {
    const reExports: any[] = [];
...
export function generateCompactReport(
    ...
    const globalFuncIndex = new Map<
        string,
        { fileId: string; funcId: string; idx: number; isExported: boolean }
    >();
            ...
            for (const exp of exports) {
                ...
                if (isReExport && funcIdx !== -1 && source) {
                    const exists = relations.reExports.some(
                        (r: any[]) =>
                            r[0] === moduleIdx && r[1] === funcIdx && r[2] === source
                    );
                    if (!exists) {
        ...
        if (includeConstants) {
            ...
            for (const constItem of consts) {
                ...
                for (const otherConst of consts) {
                    if (!otherConst || otherConst.name === constItem.name || !otherConst.name)
                        continue;
    ...
    // ============================================
    // 4.10 ✅ СБОР РЕЭКСПОРТОВ (С ГЛОБАЛЬНЫМИ КАРТАМИ)
    // ============================================

    const collectedReExports = collectReExports(entitiesMap, moduleMap, globalFuncIndex);
    ...
    if (totalFunctions > 0) {
        ...
        if (counts.length > 0) {
            avgCalls = Number(
                (counts.reduce((a, b) => a + b, 0) / totalFunctions).toFixed(2)
            );
    ...
    if (includeRelations) {
        ...
        report.graph = useCompression
            ? compressCalls(relations.calls || [])
            : relations.calls || [];
    }
    ...
    // ✅ ИСПРАВЛЕНО: безопасное сжатие путей (сохраняет flFull)
    if (useCompression) {
    ...
    if (outputPath) {
        ...
        console.log(`   📌 Экспортов функций (gr.e): ${relations.exports.length}`);
        console.log(`   📌 Реэкспортов (gr.re): ${relations.reExports.length}`);
        console.log(`   📌 Экспортов констант (gr.ce): ${relations.constExports.length}`);
        ...
        if (relations.asyncChains.length > 0) {
            ...
            if (relations.asyncChains.length > 5) {
                console.log(
                    `   ... и ещё ${relations.asyncChains.length - 5} цепочек`
                );
            }
        }
        ...
        if (relations.closures.length > 0) {
            ...
            if (relations.closures.length > 5) {
                console.log(
                    `   ... и ещё ${relations.closures.length - 5} замыканий`
                );
            }
        }
        ...
        if (relations.exports.length > 0) {
            ...
            if (relations.exports.length > 5) {
                console.log(
                    `   ... и ещё ${relations.exports.length - 5} экспортов`
                );
            }
        }
        ...
        if (relations.constExports.length > 0) {
            ...
            if (relations.constExports.length > 5) {
                console.log(
                    `   ... и ещё ${relations.constExports.length - 5} экспортов констант`
                );
            }
        }
        ...
        console.log(`      • di - динамические импорты (dynamicImports)`);
        console.log(`      • cfg - конфигурации (configRefs)`);
        console.log(`      • ext - внешние библиотеки (externalLibs)`);
        console.log(`      • vt - Vue шаблоны (vueTemplates)`);
        console.log(`      • async - асинхронные цепочки (asyncChains)`);
        console.log(`      • closures - замыкания (closures)`);
        console.log(`      • types - типовые зависимости (typeDeps)`);
        console.log(`   }`);
        console.log(`   📌 Статистика: st (расширенная)`);
        console.log(`   📌 Легенда: legend (расширенная)`);
        console.log(`   📌 Флаги: flg (FLAG_MAP)`);
    }
    ...
    for (const func of report.fns) {
        if (func[1] === name) {
            return {
                ...
                line: func[4],
                flags: func[5] || '0',
                flagNames: decodeFlagString(func[5] || '0'),
                flagObject: decodeFlagObject(func[5] || '0'),
            };
        }
    }
    return null;
...
export function getFileName(report: any, fileId: string): string | null {
    // ✅ Сначала проверяем flFull (полные пути)
    if (report.flFull && report.flFull[fileId]) {
        return report.flFull[fileId];
    }
    ...
    for (const func of report.fns) {
        if (func[0] === funcId) {
            ...
            return {
                ...
                line: func[4],
                flags: flagsStr,
                flagNames: decodeFlagString(flagsStr),
...
/**
 * @deprecated Используйте decodeFlagString
 */
export function decodeFlags(flags: string): Record<string, boolean> {
    return decodeFlagObject(flags);
}
