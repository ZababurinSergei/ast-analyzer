// src/core/re-export-resolver.ts
// ============================================
...
//
// ИЗМЕНЕНИЯ v1.0.1:
//   - ✅ ИСПРАВЛЕНО: удалён неиспользуемый параметр `currentFile`
//     из метода `expandSingle` (TS6133)
//   - ✅ ИСПРАВЛЕНО: обновлены вызовы `this.expandSingle(...)`
//     в методах `expandChain` и внутри `expandSingle`
// ============================================
...
export class CoreReExportResolver {
  ...
  /** tsconfig baseUrl (абсолютный путь) */
  ...
  /**
   * Возвращает эффективные экспорты для файла
   * (с учётом развёрнутых re-exports).
   */
  ...
  /**
   * ✅ ИСПРАВЛЕНО v1.0.1: удалён неиспользуемый параметр `currentFile`
   */
  private expandSingle(
    ...
    for (const nextRe of matching) {
      // ✅ ИСПРАВЛЕНО v1.0.1: убран второй аргумент `targetFile`
  ...
  private getReExports(filePath: string): RawReExport[] {
    ...
    for (const exp of sourceFile.getExportDeclarations()) {
      ...
      for (const named of exp.getNamedExports()) {
        ...
        result.push({
          ...
          type: originalName === 'default' ? 'default' : 'named',
          line,
          source,
        });
      }
    ...
    // 2. ExportAllDeclaration: export * from './foo' или export * as ns from './foo'
    for (const exp of sourceFile.getExportDeclarations()) {
      const source = exp.getModuleSpecifierValue();
      if (!source) continue;

      // Проверяем, есть ли namespace export: export * as ns from './foo'
