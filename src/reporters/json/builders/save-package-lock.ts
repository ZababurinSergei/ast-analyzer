...
export function savePackageLockReport(
  ...
  const { packages, exportsMap } = buildPackagesAndMaps(
    rootKey,
  ...
  // Шаг 5: Сборка финального отчёта
 ...
 */
function buildExportsForModule(
  ...
  for (const exp of entities.exports || []) {
    ...
    if (expAny.isReExport) {
      const exportKey = expAny.isStarReExport
        ? `*:${expAny.source}`
        : exp.name;
 ...
 */
function buildImportsForModule(
  entities: EntitiesResult
): Record<string, ImportEntry> {
 ...
 */
function buildFinalReport(params: BuildFinalReportParams): any {
  const {
    rootKey,
    normalizedEntitiesMap,
    packages,
...
function logReportSummary(report: any, outputPath: string): void {
  ...
  console.log(
    `💾 Size: ${((report.fileStats?.totalSize || 0) / 1024).toFixed(2)} KB`
  );
  ...
  if (report.architectureMetrics) {
    console.log(
      `🏗️  Architecture: ${report.summary?.architectureHealth || 'unknown'}`
    );
    console.log(`   📦 Vue components: ${report.architectureMetrics.vueComponents}`);
    console.log(
      `   🔄 Cycles: ${report.architectureMetrics.hasCycles ? '⚠️ YES' : '✅ NO'}`
    );
